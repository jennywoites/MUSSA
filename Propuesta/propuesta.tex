...
Considerando que debe ser al menos un cuatrimestre mayor:\\

$C_{j} \geq C_{i} + 1;  \forall$ j que tenga como correlativa a la materia i \\

Ahora bien, para este juego de ecuaciones, depende de si i o j son obligatorias o electivas:\\

- Si la materia i y la materia j son obligatorias:\\

$C_{j} \geq C_{i} + 1;  \forall$ j obligatoria que tenga como correlativa a la materia obligatoria i\\

- Si la materia i es obligatoria y la materia j es electiva:\\

$C_{j} + INFINITO * (1 - \sum_{\forall cuat \in MAX CUATRIMESTRES} Y_{j,cuat}) \geq C_{i} + 1;  \forall$ j electiva que tenga como correlativa a la materia obligatoria i \\

- Si la materia i y la materia j son electivas, se generan dos juegos de ecuaciones en lugar de uno:\\

$C_{j} + INFINITO * (1 - \sum_{\forall cuat \in MAX CUATRIMESTRES} Y_{j,cuat}) \geq C_{i} + (1 * \sum_{\forall cuat \in MAX CUATRIMESTRES} Y_{i,cuat});  \forall$ j electiva que tenga como correlativa a la materia electiva i \\

$C_{j} \leq INFINITO * \sum_{\forall cuat \in MAX CUATRIMESTRES} Y_{i,cuat};  \forall$ j electiva que tenga como correlativa a la materia electiva i \\
